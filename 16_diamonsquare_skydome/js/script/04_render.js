...
function update() {
	...
	// MOVE  Z = front , S = back , Q = left , D = right , A = up , E = down , 
	...
	if(keyboard.pressed("e")) { sprite.move("Y",-move);}
	if(keyboard.pressed("a")) { sprite.move("Y",move);}
	...
	if(mouse.state=="buttonRight") { 
			...
			if(mouse.moveX < 0 ) {sprite.rotate("X",rotate);camera.rotateOnAxis(new THREE.Vector3(0,1,0),rotate);}
			else if(mouse.moveX > 0) {sprite.rotate("X",-rotate);camera.rotateOnAxis(new THREE.Vector3(0,1,0),-rotate);	}
			mouse.moveX = 0;
		}
	
	// ZOOM wheelZoom with mouse 
